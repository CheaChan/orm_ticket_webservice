 ...
 <div class="container">
  ...
  <div class="col-md-10">
   <div class="card">
    ...
    <div class="card-body">
     <form action="{{ url('articles/store') }}/{{ isset($object) ? $object->id : 0}}" method="POST" enctype='multipart/form-data'>
      {{csrf_field()}}
        ...
        <select name="category_id" class="form-control" required>
            <option value=""> -- Please Select --</option>
                @foreach ($categories as $category)
                    <option name="category_id" value="{{$category->id}}" {{ isset($object) ? ($object->id == $category->id ? 'selected': ''): ''}}>{{ $category->title }}</option>   
                @endforeach
        </select>
      </div>
      <div class="form-group">
      ...
      <a class="btn btn-danger" role="button" href="{{url('articles')}}">Back</a>
    </form>
    </div>
   </div>
  </div>
